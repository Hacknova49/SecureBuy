...
import { SecureQR } from './frontend/src/components/SecureQR';
import { Scanner, ScanResultDisplay } from './frontend/src/components/Scanner'; 
...
import { Ticket as TicketIcon, Calendar, MapPin, ShieldCheck, LogOut, Lock, Fingerprint, ShoppingBag, AlertCircle, UserCircle, Briefcase, Plus, ArrowRight, DollarSign, Key, Check, Sparkles, Bot, ShoppingCart, Trash2, X, Map, Send } from 'lucide-react';
...
export default function App() {
    ...
    const getCartTotalPrice = () => {
        const total = Object.entries(cart).reduce((total, [id, qty]) => {
            const evt = allEvents.find(e => e.id === id);
            return total + (evt ? evt.price * qty : 0);
        }, 0);
        return Math.round(total); // Round to handle potential floating point issues
    };
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            setCartError(e.message || "Checkout failed partway through. Check limits or network.");
        } finally {
    ...
    const handleVerifyLocation = async () => {
        ...
        // In a real app, this would use the Google Maps API key (or backend proxy) to verify the address.
        // Since we are not doing a real verification, we simulate a delay.
    ...
    const handleCreateEvent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await createEvent(user.id, {
                ...
                tags: tagArray,
                // The backend will set the image and date automatically
    ...
    const openConciergeModal = () => {
        ...
        if (chatMessages.length === 0) {
            setChatMessages([{
                role: 'model',
                text: "Systems online. I am Nexus. Looking for a specific vibe or location tonight? I can also check event details.",
                timestamp: Date.now()
            }]);
        }
    };
                ...
                <div className="space-y-4">
                    {tickets.length === 0 && (
                        <div className="text-center py-20 opacity-50">
                            ...
                            <p>No tickets yet. Buy some in the Events tab!</p>
                        </div>
                    )}
                    {tickets.map(ticket => (
                        <div 
                        ...
                        {newEvent.venue && !verifyingLocation && (
                            <div className="mt-2 h-40 w-full rounded-xl overflow-hidden border border-white/10 relative group">
                                <iframe
                                    ...
                                    allowFullScreen
                                    // CORRECTED MAP URL
                                    src={`https://maps.google.com/maps?q=${encodeURIComponent(newEvent.venue)}&output=embed`}
                                ></iframe>
